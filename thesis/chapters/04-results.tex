...
Comparing the two approaches reveals distinct behaviors. The label-only model achieves faster early loss reduction but suffers from an earlier generalization peak and subsequent overfitting. In contrast, SCoTD demonstrates sustained improvement in evaluation loss, where the accuracy plateau likely reflects a reasoning ceiling rather than overfitting. Furthermore, the gradient behavior differs significantly. The high initial norms of the label-only model contrast with the moderate, steady norms of SCoTD, indicating incremental adaptation when learning longer rationales.
...
The cost distribution approximates a log-normal shape, with a long tail extending beyond \$0.00030 that consists mainly of verbose rationales. In terms of length, the median character count is around 150, while the median token count is 225. Incorrect answers tend to be mildly shorter, which may indicate information insufficiency in those responses.
