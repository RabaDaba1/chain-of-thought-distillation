...
\begin{itemize}
    \item \textbf{Optimized scraping}: Highly asynchronous dataset generator with concurrency tuned to balance throughput and timeout/error rate.
...
\begin{itemize}
    ...
    \item Format error rate (cases where numeric answer is correct but formatting invalid) kept below a small threshold (target <5\% of all generated correct numbers).
...
\begin{enumerate}
    ...
    \item \textbf{Token-length outlier removal}: Compute per-row \texttt{total\_tokens = prompt\_tokens + completion\_tokens}. Remove rows with \texttt{total\_tokens} $\ge$ 99th percentile (threshold $\approx$1392 tokens). This trims extreme verbose rationales that inflate sequence length and memory without adding diversity.
...
This section enumerates the fixed prompt templates used across generation, training, and benchmarking. Templates are versioned as plain text under \texttt{code/prompts/}. No runtime mutation other than \texttt{\{question\}} placeholder interpolation.
...
Final Answer: <number>
    ...
    \item \textbf{Label-only mode}: Single line \texttt{Final Answer: <number>} No units, commentary, or extra lines.
\end{itemize}
...
All per-question predictions across runs are concatenated and written to \texttt{predictions.csv} with columns:
\begin{itemize}
    \item \texttt{model} (run identifier)
    \item \texttt{question}
    \item \texttt{prediction} (raw decoded answer text)
    \item \texttt{gold\_answer} (numeric gold)
\end{itemize}
...
\subsection{Hardware}
\begin{itemize}
    \item Single GPU: RunPod RTX 4090 (24GB VRAM), host RAM 64GB.
    \item Pod cost: \$0.34/hour.
\end{itemize}

\subsection{Software}
\begin{itemize}
    \item Python 3.11; package manager: \texttt{uv} (\texttt{uv.lock} committed).
