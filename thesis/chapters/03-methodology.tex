...
Fixed prompt templates are used across generation, training, and benchmarking to ensure consistency. These templates are versioned as plain text files.

\subsection{Templates and formatting}
The \textbf{CoT system prompt} instructs the model to provide concise multi-step reasoning (2--8 steps) followed by a final answer line. The \textbf{CoT user prompt} provides few-shot exemplars demonstrating this format. Similarly, the \textbf{Label-only prompts} constrain the output to a single final answer line without reasoning. The formatting contract requires the final line to start exactly with "Final Answer:" followed by the number, with no trailing text.
